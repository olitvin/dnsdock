[![Build Status](https://secure.travis-ci.org/olitvin/dnsdock.png)](http://travis-ci.org/olitvin/dnsdock)
...
go get -d -v https://github.com/olitvin/dnsdock
cd ${GOPATH}/src/github.com/olitvin/dnsdock
godep restore
cd ${GOPATH}/src/github.com/olitvin/dnsdock/src
...
```
git clone https://github.com/olitvin/dnsdock <clone_directory_path>
...
```
docker run -d -v /var/run/docker.sock:/var/run/docker.sock --name dnsdock -p 172.17.0.1:53:53/udp olitvin/dnsdock [--opts]
```
...
More information in [#11](https://github.com/olitvin/dnsdock/issues/11).
...
- name: dnsdock.service
      ...
      content: |
        ...
        ExecStartPre=/bin/sh -c '/usr/bin/docker pull olitvin/dnsdock'
        ExecStart=/usr/bin/docker run -v /var/run/docker.sock:/var/run/docker.sock --name dnsdock -p ${COREOS_PRIVATE_IPV4}:53:53/udp olitvin/dnsdock
